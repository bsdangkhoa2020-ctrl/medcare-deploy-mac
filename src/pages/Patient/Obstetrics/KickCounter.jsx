...
export default function KickCounter() {
  ...
  useEffect(() => {
    const saved = localStorage.getItem('medcare_ob_kicks');
    if (saved) {
      try {
  ...
  const saveAndExit = () => {
    if (count > 0) {
      ...
      const newEntry = { date: todayStr, count, elapsed };
      // find if today already exists
      let newHistory = [...history];
      const todayIdx = newHistory.findIndex(h => h.date === todayStr);
      if (todayIdx >= 0) {
        // accumulate
        newHistory[todayIdx].count += count;
        newHistory[todayIdx].elapsed += elapsed;
      } else {
      ...
      localStorage.setItem('medcare_ob_kicks', JSON.stringify(newHistory));
  ...
  const chartData = Array.from({ length: 7 }).map((_, i) => {
    ...
    else label = d.toLocaleDateString('vi-VN', { weekday: 'short' });

    return {
      day: label,
      count: found ? found.count : 0,
      elapsed: found ? formatTime(found.elapsed) : '00:00'
    };
  }).reverse();

  const maxCount = Math.max(...chartData.map(h => h.count), 1);

  return (
    <div className="bg-[#FEFAF5] min-h-screen pb-10">
      {/* Header */}
      <div className="flex items-center justify-between px-[22px] pt-[calc(env(safe-area-inset-top)+14px)] pb-3">
        <button onClick={() => navigate('/sankhoa')} className="w-9 h-9 flex items-center justify-center text-[#1C1510] hover:bg-[#E8D8C8]/30 rounded-full transition-colors">
      ...
      <div className="pb-10">
        ...
        <div className="px-[22px] mt-6">
          <button 
            onClick={saveAndExit}
            disabled={count === 0}
            className={`w-full py-4 rounded-[14px] text-[13px] font-bold tracking-[.1em] uppercase transition-all ${
              count > 0 ? 'bg-[#B8814A] text-white shadow-md' : 'bg-[#E8D8C8] text-[#A69380] cursor-not-allowed'
            }`}
          >
            Lưu & Kết thúc
          </button>
        ...
        <div className="px-[22px] mt-7">
          ...
          <div className="flex justify-between items-end h-20 border-b-[0.5px] border-[#E8D8C8] pb-1.5">
            {chartData.map((h, i) => (
              <div key={i} className="flex flex-col items-center gap-1 flex-1 group">
                {h.count > 0 && <span className="text-[8px] text-[#735F4D] opacity-0 group-hover:opacity-100 transition-opacity">{h.count}</span>}
                <div
                  className="w-4 rounded-t-sm transition-all duration-500 ease-out"
                  style={{
                    height: `${Math.max(h.count > 0 ? 4 : 0, (h.count / maxCount) * 60)}px`,
                    background: i === 6 ? '#B8814A' : '#E8D8C8',
                  }}
                />
              </div>
            ))}
          ...
          <div className="flex justify-between mt-2">
            {chartData.map((h, i) => (
              <div key={i} className={`flex-1 text-center text-[9px] ${i === 6 ? 'text-[#B8814A] font-bold' : 'text-[#A69380]'}`}>
                {h.day === 'Hôm nay' ? 'HN' : h.day.replace('T', '')}
              </div>
            ))}
          </div>
        </div>
